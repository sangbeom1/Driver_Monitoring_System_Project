...
class ExorcistFace():
	...
	def read_exorcist_image(self, exorcist_image_url):
		self.exorcist_image = imread_from_url(exorcist_image_url)
		self.ex_im_height, self.ex_im_width, _ = self.exorcist_image.shape
	
	def detect_exorcist_mesh(self):
		# get the face mesh for the exorcist image
		landmarks = self.face_mesh.process(cv2.cvtColor(self.exorcist_image, cv2.COLOR_BGR2RGB)).multi_face_landmarks
		self.exorcist_face_coordinates = np.array(
			[[int(landmark.x * self.ex_im_width), int(landmark.y * self.ex_im_height)] for landmark in
	...
	def draw_exorcist_face(self, img, landmarks):
		...
		for face_landmarks in landmarks:
			...
			for triangle_id in range(0, len(vertices), 3):
				...
				exorcist_pix_coords = self.exorcist_face_coordinates[[corner1_id, corner2_id, corner3_id], :]
				...
				cropped_exorcist = self.exorcist_image[ex_y:ex_y + ex_h, ex_x:ex_x + ex_w]
				...
				# Warp cropped exorcist triangle into the cropped face triangle
				warp_mat = cv2.getAffineTransform(exorcist_pix_crop_coords.astype(np.float32),
				                                  face_pix_crop_coords.astype(np.float32))
				warped_triangle = cv2.warpAffine(cropped_exorcist, warp_mat, (face_w, face_h))
		...
		# Add the exorcist mask to the webcam feed
		if self.show_webcam:
...
vertices = [(127, 34), (34, 139), (139, 127), (11, 0), (0, 37), (37, 11),
